...
export default function Approvals() {
  ...
  const fetchApprovals = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/backend/approvals/approvals.php?${params.toString()}`, {
        headers: {
  ...
  const handleCreateApproval = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      const response = await fetch('http://localhost:8000/backend/approvals/approvals.php', {
        method: 'POST',
        headers: {
