...
export default function Profile() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/backend/users/profile.php?id=${user.id}`, {
        headers: {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await fetch('http://localhost:8000/backend/users/update_profile.php', {
        method: 'POST',
        headers: {
