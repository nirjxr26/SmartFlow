...
export default function Settings() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/backend/settings/settings.php?userId=${user.id}`, {
        headers: {
  ...
  const handleSaveProfile = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await fetch('http://localhost:8000/backend/settings/settings.php', {
        method: 'POST',
        headers: {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/backend/users/upload_avatar.php', {
        method: 'POST',
        headers: {
          'Authorization': localStorage.getItem('token') || '',
        },
  ...
  const togglePreference = async (key: keyof Preferences) => {
    ...
    try {
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      await fetch('http://localhost:8000/backend/settings/settings.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': localStorage.getItem('token') || '',
