...
export function TopBar({ title, subtitle, onMenuClick }: TopBarProps) {
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/backend/notifications/notifications.php?userId=${storedUser.id}&filter=unread`, {
        headers: {
  ...
  const fetchRecentNotifications = async () => {
    try {
      const storedUser = JSON.parse(localStorage.getItem('user') || '{}');
      const response = await fetch(`http://localhost:8000/backend/notifications/notifications.php?userId=${storedUser.id}&filter=all`, {
        headers: {
          'Authorization': localStorage.getItem('token') || '',
        },
      });
